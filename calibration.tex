...
% We have to consider each individually, depending on the physics we want to study. 
Looking for deviations from a modeled waveform template, we are mainly concerned with relative calibration uncertainty. In order to understand the absolute distance to the sources, we are most interested in absolute calibration. Additionally, both the detector calibration and the signal \magentacomment{Josh: signal or waveform? Could this just say, "Additionally, the waveform models have a frequency-dependent calibration error. Studies are underway to determine the exact frequency-dependant requirements on both detector and waveform calibration."} calibration are also frequency dependent. Studies are currently underway that take both aspects into account and evaluate the frequency-dependant requirements of the calibration uncertainty. 
...
The signals with the highest SNRs give the narrowest limits for GR. With a network of three 3G detectors we expect about one event per year with $\text{SNR} \sim 1000$. To avoid being dominated by calibration errors, we need a calibration \magentacomment{absolute or relative?} to an amplitude of 0.5\%.
...
Ultimately, the current photon calibrator implementation in Advanced LIGO has an absolute systematic uncertainty of 0.8\%, set mainly by a combination of uncertainty in the calibration of the NIST-traceable power standard and uncertainty in where the photon-calibrator beams and the interferometer beams impinge on the test masses~\cite{doi:10.1063/1.4967303}.
...
It is also possible to fix certain aspects of the calibration using an astrophysical source whose properties are sufficiently well known~\cite{Pitkin:2015kgm}; more work is needed to understand how this technique will inform the calibration for 3G detectors.
...
Requirements on the absolute and relative calibration for third-generation detectors are not yet fully known, and dedicated modeling efforts are needed to understand the constraints set by various 3G scientific objectives. 3G detectors may require sub-percent amplitude calibration, potentially necessitating R\&D to improve the capabilities of the current calibration apparatuses; such R\&D is already underway.