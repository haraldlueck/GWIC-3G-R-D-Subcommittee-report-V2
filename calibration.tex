...
The exact calibration requirements needed for the scientific objectives of 3G GW observatories are not yet known, but can be estimated. The calibration accuracy not only depends on the signal-to-noise ratio (SNR), but both the detector calibration and the signal calibration are also frequency dependent. Studies are currently underway that take both aspects into account and evaluate the frequency response requirements of the calibration uncertainty.  With a detector network of three 3G detectors we expect about one event per year with $\text{SNR} \sim 1000$. The signals with the highest SNRs give the narrowest limits for GR. To avoid being dominated by calibration errors, we need a calibration \magentacomment{absolute or relative?} to an amplitude of 0.5\%.
...
With $\mathcal{O}(1000)$ low-redshift binary neutron-star events, the Hubble constant may be determined to a ${\sim}1\%$ level with Einstein Telescope~\cite{Cai:2016sby}.
